...
def active_neighbors(dimension, x, y, z, w):
    result = 0
    for x_ in [x-1, x, x+1]:
        for y_ in [y-1, y, y+1]:
            for z_ in [z-1, z, z+1]:
                for w_ in [w-1, w, w+1]:
                    result += dimension[(x_, y_, z_, w_)] if (x_, y_, z_, w_) != (x, y, z, w) else 0
    return result
...
def run():
    ...
    for start_row in range(len(text_input)):
        for start_col in range(len(text_input[0])):
            pocket_dimension[(start_col, start_row, 0, 0)] = text_input[start_col][start_row] == '#'
        ...
        for x in range(-active_mass_delta, start_col + 1 + active_mass_delta):
            for y in range(-active_mass_delta, start_row + 1 + active_mass_delta):
                for z in range(-active_mass_delta, 1 + active_mass_delta):
                    ...
                    for w in range(-active_mass_delta, 1 + active_mass_delta):
                        if pocket_dimension[(x, y, z, w)]:
                            # If a cube is active and exactly 2 or 3 of its neighbors are also active, the cube remains active.
                            # Otherwise, the cube becomes inactive.
                            pocket_dimension_after_cycle[(x, y, z, w)] = active_neighbors(pocket_dimension, x, y, z, w) in [2, 3]
                        else:
                            # If a cube is inactive but exactly 3 of its neighbors are active, the cube becomes active.
                            # Otherwise, the cube remains inactive.
                            pocket_dimension_after_cycle[(x, y, z, w)] = active_neighbors(pocket_dimension, x, y, z, w) == 3
