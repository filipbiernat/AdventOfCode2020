...
def generate_regex_for_option(rule_dict, rule_option):
    ...
    if single_character_match is not None:
        ...
        return "".join(list(map(lambda rule_index: generate_regex(rule_dict, rule_index), rule_indices)))


def generate_regex(rule_dict, rule_index=0):
    # Some of the rules have multiple lists of sub-rules separated by a pipe (|).
    # This means that at least one list of sub-rules must match.
    rule_options = rule_dict[rule_index].split("|")
    regexes_for_options = list(map(lambda x: generate_regex_for_option(rule_dict, x), rule_options))
